...
describe('StellarAttestationClient', () => {
  ...
  describe('Client Initialization', () => {
    ...
    it('should handle mainnet configuration', () => {
      const mainnetClient = new StellarAttestationClient({
        rpcUrl: 'https://soroban.stellar.org',
        network: 'mainnet',
        publicKey: 'GDUMMY1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQR',
      })
      expect(mainnetClient).toBeDefined()
    })
  ...
  describe('Core Transaction Tests', () => {
    ...
    it('1. Should create a schema', async () => {
      const schemaDefinition = 'struct Identity { string name; uint32 age; bool verified; }'
      
      // Test with object-based arguments
      ...
      // For real transaction (commented out to avoid network calls in tests)
      ...
      // For testing, generate a deterministic schema UID
      schemaUid = client.generateSchemaUid({
        definition: schemaDefinition,
    ...
    it('2. Should attest to the schema', async () => {
      ...
      const tx = await client.attest({
        ...
        subject: accountKeyPair.publicKey(),
      ...
      // For testing, generate a deterministic attestation UID
      attestationUid = client.generateAttestationUid({
        schemaUid,
        subject: accountKeyPair.publicKey(),
        nonce: BigInt(1)
      })
    ...
    it('3. Should revoke the attestation', async () => {
      // Test with object-based arguments
  ...
  describe('UID Generation (Items 4-5)', () => {
    it('should generate attestation UID with object params', () => {
      const schemaUid = Buffer.alloc(32, 1)
      const subject = 'GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF'
      const nonce = BigInt(12345)
    ...
    it('should generate attestation UID with positional params', () => {
      const schemaUid = Buffer.alloc(32, 1)
      const subject = 'GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF'
      const nonce = BigInt(12345)
    ...
    it('should generate schema UID with object params', () => {
      const definition = 'name:string,age:u32'
      const authority = 'GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF'
    ...
    it('should generate schema UID with positional params', () => {
      const definition = 'name:string,age:u32'
      const authority = 'GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF'
  ...
  describe('Message Creation (Items 9-10)', () => {
    it('should create attestation message for delegation', () => {
      ...
      // WeierstrassPoint doesn't have length property, check it's the right type
  ...
  describe('Signature Verification (Item 16)', () => {
    it('should verify signature structure', () => {
      const keyPair = generateBlsKeys()
      const validSignature = Buffer.alloc(96, 1) // Mock signature
      const mockMessage = Buffer.alloc(32, 1) // Mock message point
      
      const result = client.verifySignature(
        validSignature,
        keyPair.publicKey,
        mockMessage as any // Cast to expected type for testing
      )
