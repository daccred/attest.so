...
/**
 * Validates a Solana schema object
 * @param content The schema content to validate
 * @returns Error message string or null if valid
 */
export const validateSolanaSchema = (content: any): string | null => {
  if (!content) {
  ...
  if (!content.schemaName) {
    return 'Schema must have a schemaName property'
  }

  if (!content.schemaContent) {
    return 'Schema must have a schemaContent property'
  }

  // Validate schema structure if it's meant to be a JSON schema
  if (typeof content.schemaContent === 'object') {
    return checkValidJSONContent(content.schemaContent)
  }
...
/**
 * Validates a Solana attestation object
 * @param content The attestation content to validate
 * @param schemaUid The schema UID (required reference)
 * @returns Error message string or null if valid
 */
export const validateSolanaAttestation = (content: any, schemaUid?: string): string | null => {
  if (!content) {
  ...
  if (!content.data) {
    return 'Attestation must have a data property'
  }

  // If accounts are provided, ensure they have the correct structure
  if (content.accounts) {
    if (typeof content.accounts !== 'object') {
      return 'Attestation accounts must be an object'
    }

    // Recipient is generally required
    if (!content.accounts.recipient) {
      return 'Attestation accounts must include a recipient'
    }
  }
