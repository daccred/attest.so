...
import { sorobanRpcUrl, CONTRACT_ID_TO_INDEX, MAX_EVENTS_PER_FETCH, LEDGER_HISTORY_LIMIT_DAYS } from '../common/constants';
...
export async function fetchAndStoreEvents(startLedgerFromRequest?: number): Promise<FetchEventsResult> {
  ...
  try {
    ...
    const errorMessage = `Error in fetchAndStoreEvents (events.repository.ts): ${error.message || error}`;
