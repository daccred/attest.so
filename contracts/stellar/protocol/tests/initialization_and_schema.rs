use protocol::{utils::create_xdr_string, state::Schema, AttestationContract, AttestationContractClient};
...
fn initialize_and_register_schema() {
	...
	let test_cases = [
		...
		SchemaRegistrationParams {
			name: "schema_xdr_with_resolver_and_not_revocable",
	...
	for case in test_cases {
		...
		println!("Event data: schema_uid={:?}, schema={:?}", event_data.0, event_data.1.definition);
