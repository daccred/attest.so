...
fn revoke_attestation_by_nonce() {
	...
	// register schema
	let schema_definition = SorobanString::from_str(&env, r#"{"name":"Revocable","version":"1.0","description":"Revocable","fields":[]}"#);
	...
	// revoke by attester
	env.mock_auths(&[MockAuth { address: &attester, invoke: &MockAuthInvoke { contract: &contract_id, fn_name: "revoke_attestation", args: (attester.clone(), schema_uid.clone(), subject.clone(), nonce).into_val(&env), sub_invokes: &[] } }]);
	...
	let (_schema_uid_ev, subject_ev, attester_ev, nonce_ev, revocation_time_ev): (BytesN<32>, Address, Address, u64, Option<u64>) = last.2.try_into_val(&env).unwrap();
