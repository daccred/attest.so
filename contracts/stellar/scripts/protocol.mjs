...
const PROTOCOL_CONTRACT_ID =
  envConfig.PROTOCOL_CONTRACT_ID || 'CAF5SWYR7B7V5FYUXTGYXCRUNRQEIWEUZRDCARNMX456LRD64RX76BNN'
const ADMIN_ADDRESS =
  envConfig.ADMIN_ADDRESS ||
  (ADMIN_SECRET_KEY ? Keypair.fromSecret(ADMIN_SECRET_KEY).publicKey() : null)

if (!ADMIN_SECRET_KEY) {
  console.warn(
    'Warning: ADMIN_SECRET_KEY is not set in env.sh or environment variables. Operations requiring admin privileges will fail.'
  )
}
if (!ADMIN_ADDRESS) {
  console.warn(
    'Warning: ADMIN_ADDRESS could not be derived. Please set ADMIN_SECRET_KEY or ADMIN_ADDRESS.'
  )
}
if (PROTOCOL_CONTRACT_ID === 'CAF5SWYR7B7V5FYUXTGYXCRUNRQEIWEUZRDCARNMX456LRD64RX76BNN') {
  console.warn(
    'Warning: PROTOCOL_CONTRACT_ID is using a placeholder. Please set it in env.sh or environment variables.'
  )
}
...
async function invoke({ func, args, fee = BASE_FEE }) {
  ...
  try {
    ...
    if (error.result && error.result.results) {
      console.error(
        'Result XDR:',
        error.result.results[0]?.result?.result?.result().value().toString()
      )
    } else {
  ...
  if (
    simulateResponse.status === SorobanRpc.Api.GetTransactionStatus.ERROR ||
    simulateResponse.status === SorobanRpc.Api.GetTransactionStatus.FAILED
  ) {
  ...
  try {
    ...
    if (sendResponse.status === 'PENDING') {
      ...
      while (
        txResponse.status === SorobanRpc.Api.GetTransactionStatus.PENDING &&
...
program
  .version('0.0.1')
  .description(
    'CLI utility to interact with the Attestation Protocol Soroban contract on the Stellar network.'
  )
...
program
  ...
  .option(
    '-a, --admin <address>',
    'Admin Stellar address (G...) for initialization. Defaults to ADMIN_ADDRESS from env.'
  )
  .action(async (options) => {
    ...
    try {
      ...
      if (!adminAddress) {
        throw new Error(
          'Admin address is required via --admin option or ADMIN_ADDRESS/ADMIN_SECRET_KEY in env.'
        )
      }
      ...
      console.log(
        '\nInitialize Result:',
        result ? JSON.stringify(result, null, 2) : 'Success (No return value)'
      )
    } catch (e) {
...
program
  ...
  .requiredOption(
    '-c, --caller <address>',
    'Stellar address (G...) of the account performing the registration and signing the transaction. Must match ADMIN_SECRET_KEY.'
  )
  .requiredOption(
    '-s, --schema <string>',
    'Schema definition string (e.g., "Email(string), Age(u32)").'
  )
  .option('-r, --resolver <address>', 'Optional Stellar address (G...) of a resolver contract.')
  .option(
    '--revocable',
    'Flag indicating if attestations using this schema can be revoked (defaults to false).',
    false
  )
  .action(async (options) => {
    /**
    ...
    try {
      ...
      if (options.caller !== ADMIN_ADDRESS) {
        console.warn(
          `Warning: Caller address ${options.caller} provided via --caller does not match ADMIN_ADDRESS ${ADMIN_ADDRESS} derived from ADMIN_SECRET_KEY. The transaction will be signed by ADMIN_SECRET_KEY.`
        )
      }
      ...
      console.log(
        '\nRegister Result (Schema UID is in TxMeta):',
        result
          ? JSON.stringify(result, null, 2)
          : 'Success (Check transaction meta for return value)'
      )
    } catch (e) {
...
program
  ...
  .requiredOption(
    '-c, --caller <address>',
    'Stellar address (G...) of the account performing the attestation and signing the transaction. Must match ADMIN_SECRET_KEY.'
  )
  .requiredOption(
    '--schema-uid <hex_string>',
    'Schema UID (as a 64-character hex string) obtained from registration.'
  )
  .requiredOption(
    '--subject <address>',
    'Stellar address (G...) of the subject receiving the attestation.'
  )
  .requiredOption(
    '-v, --value <string>',
    'Attestation value (string). Should match the schema definition format.'
  )
  .option(
    '--reference <string>',
    'Optional reference string to uniquely identify this attestation if multiple attestations for the same subject/schema exist.'
  )
  .action(async (options) => {
    /**
...
program
  ...
  .requiredOption(
    '-c, --caller <address>',
    'Stellar address (G...) of the account performing the revocation and signing the transaction. Must match ADMIN_SECRET_KEY.'
  )
  .requiredOption(
    '--schema-uid <hex_string>',
    'Schema UID (as a 64-character hex string) of the attestation to revoke.'
  )
  .requiredOption(
    '--subject <address>',
    'Stellar address (G...) of the subject whose attestation is being revoked.'
  )
  .option(
    '--reference <string>',
    'Optional reference string to identify the specific attestation if multiple exist.'
  )
  .action(async (options) => {
    /**
...
program
  ...
  .requiredOption(
    '--schema-uid <hex_string>',
    'Schema UID (as a 64-character hex string) of the attestation.'
  )
  .requiredOption(
    '--subject <address>',
    'Stellar address (G...) of the subject whose attestation is being retrieved.'
  )
  .option(
    '--reference <string>',
    'Optional reference string to identify the specific attestation if multiple exist.'
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.log(
        ...
        result ? JSON.stringify(result, null, 2) : 'Not Found or Error'
      )
    } catch (e) {
