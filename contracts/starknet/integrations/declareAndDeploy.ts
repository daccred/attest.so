...
async function main() {
  ...
  const provider = new RpcProvider({
    nodeUrl: `https://starknet-sepolia.infura.io/v3/${process.env.STARKNET_API_KEY}`,
  }) // only for starknet-devnet-rs
  ...
  const testSierra = json.parse(
    fs.readFileSync('./target/dev/starknet_attestso_SAS.contract_class.json').toString('ascii')
  )
  const testCasm = json.parse(
    fs
      .readFileSync('./target/dev/starknet_attestso_SAS.compiled_contract_class.json')
      .toString('ascii')
  )
