...
async function main(contract_name: string) {
  ...
  const provider = new RpcProvider({
    nodeUrl: `https://starknet-sepolia.g.alchemy.com/v2/${process.env.ALCHEMY_STARKNET_API_KEY}`,
  })
  ...
  const testSierra = json.parse(
    fs
      .readFileSync(`./target/dev/starknet_attestso_${contract_name}.contract_class.json`)
      .toString('ascii')
  )
  const testCasm = json.parse(
    fs
      .readFileSync(`./target//dev/starknet_attestso_${contract_name}.compiled_contract_class.json`)
      .toString('ascii')
  )
